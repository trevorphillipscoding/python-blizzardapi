...
class Diablo3CommunityApi(Api):
    ...
    def get_recipe(self, region, locale, artisan_slug, recipe_slug):
        """D3 Artisan and Recipe API - Returns a single recipe by slug for the specified artisan."""
    ...
    def get_character_class(self, region, locale, class_slug):
        """D3 Character Class and Skill API - Returns a single character class by slug."""
        resource = f"/d3/data/hero/{class_slug}"
        query_params = {"locale": locale}
        return super().get_resource(resource, region, query_params)

    def get_api_skill(self, region, locale, class_slug, skill_slug):
        """D3 Character Class and Skill API - Returns a single skill by slug for a specific character class."""
